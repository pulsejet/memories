OC.L10N.register(
    "memories",
    {
    "Shared Link" : "Общая ссылка",
    "Download" : "Скачать",
    "Memories" : "Воспоминания",
    "Fast, modern and advanced photo management suite" : "Быстрый, современный и мощный менеджер фотографий",
    "# Memories: Photo Management for Nextcloud\n\nMemories is a *batteries-included* photo management solution for Nextcloud with advanced features including:\n\n- **📸 Timeline**: Sort photos and videos by date taken, parsed from Exif data.\n- **⏪ Rewind**: Jump to any time in the past instantly and relive your memories.\n- **🤖 AI Tagging**: Group photos by people and objects, powered by [recognize](https://github.com/nextcloud/recognize) and [facerecognition](https://github.com/matiasdelellis/facerecognition).\n- **🖼️ Albums**: Create albums to group photos and videos together. Then share these albums with others.\n- **🫱🏻‍🫲🏻 External Sharing**: Share photos and videos with people outside of your Nextcloud instance.\n- **📱 Mobile Support**: Work from any device, of any shape and size through the web app.\n- **✏️ Edit Metadata**: Edit dates and other metadata on photos quickly and in bulk.\n- **📦 Archive**: Store photos you don't want to see in your timeline in a separate folder.\n- **📹 Video Transcoding**: Transcode videos and use HLS for maximal performance.\n- **🗺️ Map**: View your photos on a map, tagged with accurate reverse geocoding.\n- **📦 Migration**: Migrate easily from Nextcloud Photos and Google Takeout.\n- **⚡️ Performance**: Do all this very fast.\n\n## 🚀 Installation\n\n1. Install the app from the Nextcloud app store (try a demo [here](https://demo.memories.gallery/apps/memories/)).\n1. Perform the recommended [configuration steps](https://memories.gallery/config/).\n1. Run `php occ memories:index` to generate metadata indices for existing photos.\n1. Open the 📷 Memories app in Nextcloud and set the directory containing your photos." : "# Memories: управление фотографиями для Nextcloud\n\nMemories — это решение для управления фотографиями *с батарейками* для Nextcloud с расширенными функциями, включая:\n\n- **📸 Временная шкала**: сортировка фотографий и видео по дате съемки, проанализированная из данных Exif.\n- **⏪ Перемотка**: мгновенный переход в любое время в прошлом и повторное переживание воспоминаний.\n- **🤖 Тегирование ИИ**: группировка фотографий по людям и объектам с помощью [recognize](https://github.com/nextcloud/recognize) и [facerecognition](https://github.com/matiasdelellis/facerecognition).\n- **🖼️ Альбомы**: создание альбомов для группировки фотографий и видео. Затем делитесь этими альбомами с другими.\n- **🫱🏻‍🫲🏻 Внешний обмен**: делитесь фотографиями и видео с людьми за пределами вашего экземпляра Nextcloud.\n- **📱 Поддержка мобильных устройств**: работайте с любого устройства, любой формы и размера через веб-приложение.\n- **✏️ Редактирование метаданных**: быстрое и массовое изменение дат и других метаданных на фотографиях.\n- **📦 Архив**: храните фотографии, которые вы не хотите видеть на своей временной шкале, в отдельной папке.\n- **📹 Транскодирование видео**: транскодируйте видео и используйте HLS для максимальной производительности.\n- **🗺️ Карта**: просматривайте свои фотографии на карте, помеченные точным обратным геокодированием.\n- **📦 Миграция**: легко переносите из Nextcloud Photos и Google Takeout.\n- **⚡️ Производительность**: делайте все это очень быстро.\n\n## 🚀 Установка\n\n1. Установите приложение из магазина приложений Nextcloud (попробуйте демо-версию [здесь](https://demo.memories.gallery/apps/memories/)).\n1. Выполните рекомендуемые [шаги по настройке](https://memories.gallery/config/).\n1. Запустите `php occ memories:index`, чтобы сгенерировать индексы метаданных для существующих фотографий.\n1. Откройте приложение 📷 Memories в Nextcloud и укажите каталог, содержащий ваши фотографии.",
    "People (Recognize)" : "Люди (Распознавание)",
    "People" : "Люди",
    "People (Face Recognition)" : "Люди (Распознавание лиц)",
    "Timeline" : "Лента",
    "Explore" : "Обзор",
    "Folders" : "Папки",
    "Favorites" : "Избранное",
    "Albums" : "Альбомы",
    "Archive" : "Архив",
    "On this day" : "В этот день",
    "Places" : "Места",
    "Map" : "Карта",
    "Tags" : "Метки",
    "Settings" : "Настройки",
    "View all" : "Просмотреть все",
    "Videos" : "Видео",
    "Recognize" : "Распознавание",
    "Face Recognition" : "Распознавание лиц",
    "Choose the root of your timeline" : "Выберите основную папку Ленты",
    "The selected folder does not seem to be valid. Try again." : "Выбранная папка не может быть использована. Выберите другую папку.",
    "_Found {n} item in {path}_::_Found {n} items in {path}_" : ["В размещении «{path}» найден {n} объект.","В размещении «{path}» найдено {n} объекта.","В размещении «{path}» найдено {n} объектов.","В размещении «{path}» найдено {n} объекта."],
    "No photos were found in the selected folder." : "В выбранной папке нет ни одной фотографии.",
    "This can happen because your media is still indexing." : "Причиной этого может являться выполняющееся индексирование файлов.",
    "Visit the admin panel to make sure Memories is configured correctly." : "Перейдите панель администрирования, чтобы убедиться, что приложение настроено верно.",
    "A better photos experience awaits you" : "Вас ждут лучшие фотографии",
    "Choose the root folder of your timeline to begin" : "Чтобы начать, выберите корневую папку для Ленты",
    "Continue to Memories" : "Перейти к воспоминаниям",
    "Choose again" : "Выберите еще раз",
    "Click here to start" : "Нажмите здесь чтобы начать",
    "You can always change this later in settings" : "Эти параметры могут быть позднее изменены в разделе «Настройки»",
    "If you just installed Memories, visit the admin panel first." : "Перейдите в панель администрирования перед использованием приложения.",
    "No coordinates" : "Координаты отсуствуют",
    "Click edit to set location" : "Нажмите чтобы указать местоположение",
    "Shared By" : "Доступ предоставлен",
    "Metadata" : "Метаданные",
    "Edit" : "Редактировать",
    "Failed to load metadata" : "Ошибка загрузки метаданных",
    "Photos" : "Фотографии",
    "Delete" : "Удалить",
    "Remove from album" : "Удалить из альбома",
    "Share" : "Поделиться",
    "Favorite" : "В избранное",
    "Unarchive" : "Восстановить из архива",
    "Edit metadata" : "Редактировать метаданные",
    "Rotate / Flip" : "Вращать / перевернуть",
    "View in folder" : "Посмотреть в папке",
    "Set as cover image" : "Установить на обложку",
    "Move to folder" : "Переместить в папку",
    "Add to album" : "Добавить в альбом",
    "Move to person" : "Назначить метку лица",
    "Remove from person" : "Снять метку лица",
    "You must enable \"Mark person in preview\" to use this feature" : "Чтобы использовать эту функцию, вам необходимо включить «Отметить человека в предварительном просмотре».",
    "Only user \"{user}\" can update this person" : "Данные этого человека может изменить только пользователь {user}",
    "Cancel" : "Отменить",
    "_{n} selected_::_{n} selected_" : ["Выбрано {n} изображение","Выбраны {n} изображения","Выбрано {n} изображений","Выбраны {n} изображения"],
    "Memories Settings" : "Параметры приложения",
    "General" : "Основные",
    "Photo Viewer" : "Просмотр фотографий",
    "Account" : "Учётная запись",
    "Choose Timeline Paths" : "Расположения фотографий Ленты",
    "Choose the root for the folders view" : "Выберете корневую папку с фотографиями",
    "Sign out" : "Выйти",
    "Are you sure you want to log out {user}?" : "Вы уверены, что хотите выйти из системы {user}?",
    "Timeline Path" : "Расположение Ленты",
    "Square grid mode" : "Квадратные миниатюры",
    "Show past photos on top of timeline" : "Сортировать фото Ленты от старых к новым",
    "Stack RAW files with same name" : "RAW файлы с таким же именем",
    "De-duplicate identical files" : "Удалять дубликаты идентичных файлов",
    "Show photo owner name on timeline" : "Показывать имя автора фото в Ленте",
    "Autoplay Live Photos" : "Автоматически воспроизводить живые фотографии",
    "Loop Live Photos" : "Повторять живые фотографии",
    "Loop Videos" : "Воспроизводить по кругу ",
    "Show full file path in sidebar" : "Показывать полный путь в боковой панели",
    "Show metadata in slideshow" : "Показывать метаданные в слайд шоу",
    "High resolution image loading behavior" : "Обработка изображений в высоком разрешении",
    "Load high resolution image on zoom" : "Загружать изображение в высоком разрешении при приближении",
    "Always load high resolution image (not recommended)" : "Всегда загружать изображение в высоком разрешении (не рекомендуется)",
    "Never load high resolution image" : "Не загружать изображения в высоком разрешении",
    "Logged in as {user}" : "Вошёл как {user}",
    "Device Folders" : "Папки устройства",
    "Local folders to include in the timeline view" : "Папки на устройстве для отображения в Ленте",
    "Run initial device setup" : "Запустите первоначальную настройку устройства",
    "Folders Path" : "Верхний уровень для папок",
    "Show hidden folders" : "Показывать скрытые файлы",
    "Sort folders oldest-first" : "Начинать ленту с более старых папок",
    "Sort albums oldest-first" : "Начинать ленту с более старых альбомов",
    "Show hidden albums" : "Показывать скрытые альбомы",
    "Info" : "Информация",
    "Close" : "Закрыть",
    "{photoCount} photos" : "фотографий: {photoCount}",
    "Failed to load some photos" : "Не удалось загрузить некоторые фотографии",
    "Failed to update setting" : "Ошибка при сохранении новых параметров",
    "Recommended Apps" : "Рекомендуемые приложения",
    "Albums support is enabled through the Photos app." : "Поддержка альбомов включается через приложение «Фото».",
    "Albums are disabled because the Photos app is not available." : "Альбомы отключены, поскольку приложение «Фотографии» недоступно.",
    "Recognize is installed and enabled for face recognition." : "Распознавание установлено и включено для распознавания лиц.",
    "Recognize is installed but not enabled for face recognition." : "Программа «Распознавание» установлена, но не включена для распознавания лиц.",
    "Recognize is not installed. Face recognition and object tagging may be unavailable." : "Распознавание не установлено. Распознавание лиц и маркировка объектов могут быть недоступны.",
    "Face Recognition is installed and enabled" : "Распознавание лиц установлено и включено.",
    "Preview generator is installed and enabled. Additional configuration may still be required." : "Генератор предварительного просмотра установлен и включен. Может потребоваться дополнительная настройка.",
    "Preview generator is not installed and configured. This may make Memories very slow." : "Генератор предварительного просмотра не установлен и не настроен. Это может сделать Воспоминания очень медленными.",
    "EXIF Extraction" : "EXIF извлечение",
    "Path to packaged exiftool binary" : "Путь к упакованному двоичному файлу exiftool",
    "You need perl only if the packaged exiftool binary does not work for some reason." : "Perl вам понадобится только в том случае, если упакованный двоичный файл exiftool по какой-то причине не работает.",
    "Use system perl (only if exiftool binary does not work)" : "Используйте системный Perl (только если двоичный файл exiftool не работает)",
    "File Support" : "Поддержка файлов",
    "Images (JPEG, PNG, GIF, BMP)" : "Изображения (JPEG, PNG, GIF, BMP)",
    "HEIC (Imagick)" : "HEIC (Imagick)",
    "TIFF (Imagick)" : "HEIC (Imagick)",
    "Videos (ffmpeg)" : "Видео (ffmpeg)",
    "Imaginary (not recommended)" : "Imaginary (не рекомендуется)",
    "You can configure the enabled Nextcloud preview providers below." : "Ниже вы можете настроить включенных поставщиков предварительной версии Nextcloud.",
    "If you are using Imaginary for preview generation, you can ignore this section." : "Если вы используете Imaginary для создания предварительного просмотра, вы можете игнорировать этот раздел.",
    "To enable RAW support, install the Camera RAW Previews app." : "Чтобы включить поддержку RAW, установите приложение Camera RAW Previews.",
    "Documentation." : "Документация.",
    "PHP-Imagick is available [{version}]." : "PHP-Imagick доступен [{version}].",
    "PHP-Imagick is not available." : "PHP-Imagick недоступен.",
    "Image editing will not work correctly." : "Редактирование изображений не будет работать корректно.",
    "Thumbnail generation may not work for some formats (HEIC, TIFF)." : "Создание миниатюр может не работать для некоторых форматов (HEIC, TIFF).",
    "Thumbnails for videos will be generated with this binary." : "Миниатюры видео будут созданы с помощью этого двоичного файла.",
    "Thumbnail generation may not work for videos." : "Генерация миниатюр может не работать для видео.",
    "Imaginary is enabled for preview generation. This will override other preview providers. We currently recommend against using Imaginary due to multiple bugs in handling of HEIC and GIF files." : "Для создания предварительных просмотров включена функция Imaginary. Это переопределит другие функции предварительных просмотров. В настоящее время мы не рекомендуем использовать Imaginary из-за множества ошибок при обработке файлов HEIC и GIF.",
    "The following MIME types are configured for preview generation." : "Следующие типы MIME настроены для создания предварительного просмотра.",
    "Max preview size (trade-off between quality and storage requirements)." : "Максимальный размер предварительного просмотра (компромисс между качеством и требованиями к объему хранилища).",
    "Max memory for preview generation (MB)" : "Максимальный объем памяти для создания предварительного просмотра (МБ)",
    "Max size of file to generate previews for (MB)" : "Максимальный размер файла для создания предварительного просмотра (МБ)",
    "Help & Support" : "Справка и поддержка",
    "Thank you for choosing Nextcloud and Memories to store your precious data!" : "Благодарим вас за выбор Nextcloud и Memories для хранения ваших ценных данных!",
    "Memories is very feature rich and setting it up properly can take some time." : "Программа «Воспоминания» очень многофункциональна, и ее правильная настройка может занять некоторое время.",
    "If you just installed Memories, make sure you read the getting started guide:" : "Если вы только что установили Memories, обязательно прочтите руководство по началу работы:",
    "External Link" : "Внешняя ссылка",
    "In case you run into any issues or bugs, you can get help through several channels." : "Если у вас возникнут какие-либо проблемы или ошибки, вы можете получить помощь по нескольким каналам.",
    "Memories is a completely free and open source app under active development." : "Memories — это совершенно бесплатное приложение с открытым исходным кодом, находящееся в активной разработке.",
    "You can contribute in several ways. See the project page for more details:" : "Вы можете внести свой вклад несколькими способами. Более подробную информацию смотрите на странице проекта:",
    "Media Indexing" : "Индексация медиа",
    "{n} media files have been indexed" : "Проиндексировано файлов: {n}",
    "{n} media files failed indexing and were skipped." : "{n} файл(ы) не прошли индексацию и были пропущены.",
    "View failure logs." : "Открыть журнал ошибок.",
    "Files that failed indexing will not be indexed again unless they change." : "Файлы, не прошедшие индексацию, не будут индексироваться повторно, если они не изменятся.",
    "You can manually retry files that failed indexing." : "Вы можете вручную повторить попытку индексирования файлов, которым не удалось выполнить индексацию.",
    "Automatic Indexing status: {status}" : "Состояние автоматического индексирования: {status}",
    "Last index job was run {t} seconds ago." : "Последнее задание индексирования было выполнено {t} секунд назад.",
    "It took {t} seconds to complete." : "На выполнение ушло {t} секунд",
    "It is still running or was interrupted." : "Он все еще работает или был прерван.",
    "Looks like it has been more than an hour since the last index job was run. Make sure Nextcloud cron is configured correctly." : "Похоже, прошло больше часа с момента запуска последнего задания индексирования. Убедитесь, что Nextcloud cron настроен правильно.",
    "Only server-side encryption (OC_DEFAULT_MODULE) is supported, but another encryption module is enabled." : "Поддерживается только шифрование на стороне сервера (OC_DEFAULT_MODULE), но включен другой модуль шифрования.",
    "The EXIF indexes are built and checked in a periodic background task. Be careful when selecting anything other than automatic indexing. For example, setting the indexing to only timeline folders may cause delays before media becomes available to users, since the user configures the timeline only after logging in." : "Индексы EXIF создаются и проверяются периодически в фоновой задаче. Будьте осторожны при выборе чего-либо, кроме автоматического индексирования. Например, установка индексации только для папок Ленты может привести к задержкам перед тем, как медиафайлы станут доступны пользователям, поскольку пользователь настраивает Ленту только после входа в систему.",
    "Index all media automatically (recommended)" : "Автоматически индексировать все медиафайлы (рекомендуется)",
    "Index per-user timeline folders (not recommended)" : "Индексировать папки Ленты для каждого пользователя (не рекомендуется)",
    "Index a fixed relative path" : "Индексировать фиксированный относительный путь",
    "Disable background indexing" : "Отключить фоновую индексацию",
    "Indexing path (relative, all users)" : "Путь индексирования (относительный, все пользователи)",
    "Folders with a \".nomedia\" or a \".nomemories\" file are always excluded from indexing." : "Папки с файлом «.nomedia» или «.nomemories» всегда исключаются из индексирования.",
    "You can optionally use a regular expression to exclude matching paths from being indexed." : "Вы можете использовать регулярное выражение, чтобы исключить индексацию совпадающих путей.",
    "For example, to exclude special QNAP folders:" : "Например, чтобы исключить специальные папки QNAP:",
    "Or, exclude all files starting with \"private-\" or \"backup-\":" : "Или исключите все файлы, начинающиеся с \"private-\" или \"backup-\":",
    "You can use the regex101 website to validate and test the pattern:" : "Вы можете использовать сайт regex101 для проверки и тестирования шаблона:",
    "Exclude paths matching regular expression" : "Исключить пути, соответствующие регулярному выражению",
    "For advanced usage, perform a run of indexing by running:" : "Для расширенного использования выполните индексирование, выполнив:",
    "Run index in parallel with 4 threads:" : "Запуск индекса параллельно с 4 потоками:",
    "Force re-indexing of all files:" : "Принудительно переиндексировать все файлы:",
    "You can limit indexing by user and/or folder:" : "Вы можете ограничить индексацию по пользователю и/или папке:",
    "Clear all existing index tables:" : "Очистите все существующие индексные таблицы:",
    "Performance" : "Производительность",
    "Unknown" : "Неизвестно",
    "HTTPS is enabled" : "Поддержка HTTPS включена",
    "You are accessing this page over an insecure context. Several browser APIs are not available, which will make Memories very slow. Enable HTTPS on your server to improve performance." : "Вы получаете доступ к этой странице в незащищенном контексте. Некоторые API-интерфейсы браузера недоступны, из-за чего Memories будет работать очень медленно. Включите HTTPS на вашем сервере, чтобы повысить производительность.",
    "HTTP/2 or HTTP/3 is enabled" : "HTTP/2 или HTTP/3 включен",
    "HTTP/2 or HTTP/3 is strongly recommended ({httpVer} detected)" : "Настоятельно рекомендуется использовать HTTP/2 или HTTP/3 (обнаружен {httpVer})",
    "Database triggers are set up correctly." : "Триггеры базы данных настроены правильно.",
    "Database triggers not set up; {m} mode in use." : "Триггеры базы данных не настроены;  используемый режим {m}.",
    "See the documentation for information on how to resolve this." : "Смотрите документацию для получения информации о том, как решить эту проблему.",
    "Reverse Geocoding" : "Обратное геокодирование",
    "Geometry support was not detected in your database" : "В вашей базе данных не обнаружена поддержка геометрии",
    "MySQL-like geometry support was detected " : "Обнаружена поддержка MySQL-подобной геометрии",
    "Postgres native geometry support was detected" : "Обнаружена собственная поддержка геометрии Postgres",
    "Looks like the database is already setup. Are you sure you want to redownload planet data?" : "Похоже, база данных уже настроена. Вы уверены, что хотите заново загрузить данные о планете?",
    "You are about to download the planet database. This may take a while." : "Вы собираетесь загрузить базу данных планет. Это может занять некоторое время.",
    "This may also cause all photos to be re-indexed!" : "Это также может привести к переиндексации всех фотографий!",
    "Download planet database" : "Загрузить базу данных планеты",
    "Continue" : "Продолжить",
    "Database is populated with {n} geometries." : "База данных заполнена {n} геометриями.",
    "Geometry table has not been created." : "Таблица геометрии не создана.",
    "Looks like the planet data is incomplete." : "Похоже, данные о планете неполные.",
    "Reverse geocoding has not been configured ({status})." : "Обратное геокодирование не настроено ({status}).",
    "Memories supports offline reverse geocoding using the OpenStreetMaps data on MySQL and Postgres." : "Memories поддерживает автономное обратное геокодирование с использованием данных OpenStreetMaps в MySQL и Postgres.",
    "You need to download the planet data into your database. This is highly recommended and has low overhead." : "Вам необходимо загрузить данные о планете в вашу базу данных. Это настоятельно рекомендуется и имеет низкие расходы.",
    "If the button below does not work for importing the planet data, use the following command:" : "Если кнопка ниже для импорта данных планеты не работает, используйте следующую команду:",
    "Note: the geometry data is stored in the memories_planet_geometry table, with no prefix." : "Примечание. Данные геометрии хранятся в таблице memory_planet_geometry без префикса.",
    "Video Streaming" : "Видео трансляция",
    "Live transcoding provides for adaptive streaming of videos using HLS." : "Живое транскодирование обеспечивает адаптивную потоковую передачу видео с использованием HLS.",
    "Note that this may be very CPU intensive without hardware acceleration, and transcoding will not be used for external storage." : "Обратите внимание, что это может сильно нагружать процессор без аппаратного ускорения, и перекодирование не будет использоваться для внешнего хранилища.",
    "Enable Transcoding" : "Включить транскодирование",
    "ffmpeg path" : "путь ffmpeg",
    "ffprobe path" : "путь ffprobe",
    "Global default video quality (user may override)" : "Качество видео по умолчанию (пользователь может изменить значение)",
    "Auto (adaptive transcode)" : "Авто (адаптивное транскодирование)",
    "Original (transcode with max quality)" : "Оригинал (перекодировать с максимальным качеством)",
    "Direct (original video file without transcode)" : "Direct (исходный видеофайл без перекодирования)",
    "HW Acceleration" : "HW ускорение",
    "VA-API device ({dev}) is readable" : "Устройство VA-API ({dev}) доступно для чтения",
    "VA-API device ({dev}) not found" : "Устройство VA-API ({dev}) не найдено",
    "VA-API device ({dev}) has incorrect permissions" : "Устройство VA-API ({dev}) не корректные права",
    "VA-API device status: {status}" : "Статус устройства VA-API: {status}",
    "You must first make sure the correct drivers are installed before configuring acceleration." : "Прежде чем настраивать ускорение, необходимо убедиться, что установлены правильные драйверы.",
    "Make sure you test hardware acceleration with various options after enabling." : "После включения обязательно протестируйте аппаратное ускорение с различными опциями.",
    "Do not enable multiple types of hardware acceleration simultaneously." : "Не включайте одновременно несколько типов аппаратного ускорения.",
    "Intel processors supporting QuickSync Video (QSV) as well as some AMD GPUs can be used for transcoding using VA-API acceleration." : "Процессоры Intel, поддерживающие QuickSync Video (QSV), а также некоторые графические процессоры AMD можно использовать для перекодирования с помощью ускорения VA-API.",
    "For more details on driver installation, check the documentation:" : "Более подробную информацию об установке драйвера смотрите в документации:",
    "Enable acceleration with VA-API" : "Включить ускорение с помощью VA-API",
    "Enable low-power mode (QSV only)" : "Включить режим низкого энергопотребления (только QSV)",
    "NVIDIA GPUs can be used for transcoding using the NVENC encoder with the proper drivers." : "Видеокарты NVIDIA можно использовать для перекодирования с помощью кодировщика NVENC при наличии соответствующих драйверов.",
    "Depending on the versions of the installed SDK and ffmpeg, you need to specify the scaler to use" : "В зависимости от версий установленного SDK и ffmpeg вам нужно указать используемый масштабировщик",
    "No automated tests are available for NVIDIA acceleration." : "Автоматические тесты для ускорения NVIDIA недоступны.",
    "Enable acceleration with NVENC" : "Включить ускорение с помощью NVENC",
    "Enable NVENC Temporal AQ" : "Включить NVENC Temporal AQ",
    "CUDA scaler" : "Масштабировщик CUDA",
    "NPP scaler" : "Масштабировщик NPP",
    "Due to a bug in certain hardware drivers, videos may appear in incorrect orientations when streaming. This can be resolved in some cases by rotating the video on the accelerator." : "Из-за ошибки в некоторых драйверах оборудования видео может отображаться в неправильной ориентации при потоковой передаче. В некоторых случаях это можно исправить, повернув видео на ускорителе.",
    "Some drivers (e.g. AMD and older Intel) do not support hardware accelerated rotation. You can attempt to force software-based transpose in this case." : "Некоторые драйверы (например, AMD и более старые версии Intel) не поддерживают аппаратное ускорение поворота. В этом случае вы можете попытаться принудительно выполнить поворот программным способом.",
    "Try this option only if you have incorrectly oriented videos during playback." : "Используйте этот вариант только в том случае, если у вас видео с неправильной ориентацией во время воспроизведения.",
    "Enable streaming transpose workaround" : "Включить обходной путь потокового транспонирования",
    "Force transpose in software" : "Принудительное транспонирование в программном обеспечении",
    "Some NVENC devices have issues with force_key_frames." : "На некоторых устройствах NVENC возникают проблемы с force_key_frames.",
    "Try this option only if you use NVENC and have issues with video playback." : "Попробуйте этот вариант, только если вы используете NVENC и у вас возникают проблемы с воспроизведением видео.",
    "Enable streaming GOP size workaround" : "Включить обходной путь для размера GOP в потоковой передаче",
    "Transcoder" : "Транскодирование",
    "Memories uses the go-vod transcoder. You can run go-vod exernally (e.g. in a separate Docker container for hardware acceleration) or use the built-in transcoder. To use an external transcoder, enable the following option and follow the instructions in the documentation:" : "Воспоминания использует транслятор go-vod. Вы можете запустить go-vod извне (например, в отдельном контейнере Docker для аппаратного ускорения) или использовать встроенный транслятор. Чтобы использовать внешний транслятор, включите следующую опцию и следуйте инструкциям в документации:",
    "Enable external transcoder" : "Включить внешний транскодер",
    "Binary path (local only)" : "Двоичный путь (только локальный)",
    "Bind address (local only)" : "Адрес привязки (только локальный)",
    "Connection address (same as bind if local)" : "Адрес подключения (совпадает с адресом привязки, если он локальный)",
    "Quality Factor (15 - 45) (default 25)" : "Коэффициент качества (15–45) (по умолчанию 25)",
    "Default high resolution image loading behavior of the photo viewer." : "Режим загрузки изображений с высоким разрешением в программе просмотра фотографий по умолчанию.",
    "The configuration here also applies to public link shares." : "Приведенная здесь конфигурация также применима к общедоступным ссылкам.",
    "Users may override this setting." : "Пользователи могут переопределить этот параметр.",
    "Always load high resolution image (not recommended if using HEIC/TIFF)" : "Всегда загружать изображение с высоким разрешением (не рекомендуется при использовании HEIC/TIFF)",
    "Template" : "Шаблон",
    "Shared" : "Опубликованное",
    "Search your photos …" : "Поиск по фотографиям…",
    "Start typing to find photos and albums" : "Начните печатать, чтобы найти фотографии и альбомы",
    "No results found" : "Результаты отсутствуют",
    "Search" : "Поиск",
    "Upload" : "Отправить",
    "_{n} photo updated_::_{n} photos updated_" : ["{n} фото обновлено","обновлено {n} фотографий","обновлено {n} фотографий","обновлено {n} фотографий"],
    "Failed to fetch collaborators list." : "Не удалось получить список соавторов.",
    "Public link" : "Общедоступная ссылка",
    "Failed to fetch album." : "Не удалось получить альбом.",
    "Failed to update album." : "Не удалось изменить альбом.",
    "Add people or groups who can edit your album" : "Добавьте пользователей или группы, которые могут редактировать альбом",
    "Search for collaborators" : "Поиск людей и групп для добавления в соавторы",
    "Search people or groups" : "Введите имя пользователя или название группы…",
    "Add {collaboratorLabel} to the collaborators list" : "Добавить {collaboratorLabel} в соавторы",
    "No collaborators available" : "Нет ни одного возможного соавтора",
    "Remove {collaboratorLabel} from the collaborators list" : "Убрать {collaboratorLabel} из списка соавторов ",
    "Copy the public link" : "Скопировать общедоступную ссылку",
    "Public link copied!" : "Общедоступная ссылка скопирована",
    "Copy public link" : "Скопировать общедоступную ссылку",
    "Delete the public link" : "Удалить общедоступную ссылку",
    "Share via public link" : "Поделиться общедоступной ссылкой",
    "Could not load the selected album" : "Не удалось загрузить выбранный альбом",
    "Create new album" : "Создать альбом",
    "Edit album details" : "Изменить альбом",
    "New album" : "Создать альбом",
    "Failed to delete {name}." : "Не удалось удалить «{name}».",
    "Remove Album" : "Удалить альбом",
    "Leave Album" : "Отказаться от доступа",
    "Are you sure you want to permanently remove album \"{name}\"?" : "Подтвердите безвозвратное удаление альбома «{name}»",
    "Are you sure you want to leave the shared album \"{name}\"?" : "Подтвердите отказ от доступа к альбому «{name}»",
    "Save" : "Сохранить",
    "Create album" : "Создать альбом",
    "Invalid album name; should not contain any slashes." : "Имя альбома не должно содержать символа косой черты.",
    "Album Name" : "Название альбома",
    "Location" : "Местоположение",
    "Location of the album" : "Геопозиция",
    "Go back to the previous view." : "Вернуться к предыдущему режиму просмотра.",
    "Back" : "Назад",
    "Go to the add collaborators view." : "Перейти к просмотру соавторов.",
    "Add collaborators" : "Добавить соавторов",
    "Back to the new album form." : "Вернуться к диалогу создания альбома.",
    "Create new album." : "Создать альбом",
    "Save changes" : "Сохранить изменения",
    "_Removed from {n} album_::_Removed from {n} albums_" : ["Удалить {n} из альбома","Удалить {n} из альбома","Удалить {n} из альбома","Удалить {n} из альбома"],
    "Failed to load album info: {name}" : "Не удалось загрузить информацию об альбоме: {name}",
    "Share Album" : "Опубликовать альбом",
    "Name of the album" : "Название",
    "Save collaborators for this album." : "Сохранить список соавторов альбома.",
    "Invalid Date" : "Недопустимая дата",
    "Newest date is older than oldest date" : "Более новая дата ранее более старой",
    "Newest" : "Более новые",
    "Year" : "Год",
    "Month" : "Месяц",
    "Day" : "День",
    "Hour" : "Час",
    "Minute" : "Минута",
    "Oldest" : "Более старые",
    "Title" : "Название",
    "Description" : "Описание",
    "Label" : "Метка",
    "Camera Make" : "Производитель камеры",
    "Camera Model" : "Модель камеры",
    "Lens Model" : "Модель линз",
    "Copyright" : "Авторские права",
    "Empty" : "Пустой",
    "Unchanged" : "Неизменившийся",
    "Failed to search for location with Nominatim." : "Не удалось найти местоположение в службе Nominatim.",
    "Reset" : "Сброс",
    "Remove location" : "Удалить сведения о местоположении",
    "Search location / landmark" : "Поиск места или достопримечательности",
    "Cannot edit {name} of type {type}" : "Не удается изменить {name} типа {type}",
    "Cannot edit rotation on videos ({name})" : "Не удается повернуть видео ({name})",
    "Cannot edit rotation on Live Photos ({name})" : "Не удается повернуть живое фото ({name})",
    "Some selected items have stacked RAW files.\nRAW files will not be edited." : "Некоторые выбранные элементы содержат файлы RAW.\nФайлы RAW редактироваться не будут.",
    "Missing date metadata" : "Отсутствуют метаданные о дате",
    "Some items may be missing the date metadata. Do you want to attempt copying the currently known timestamp to the metadata (recommended)? Othewise, the timestamp may be reset to the current time." : "В некоторых элементах могут отсутствовать метаданные о дате. Вы хотите попытаться скопировать известную на данный момент метку времени в метаданные (рекомендуется)? В противном случае метка времени может быть сброшена до текущего времени.",
    "Failed to load metadata for {n} photos." : "Не удалось загрузить метаданные из {n} фотографий.",
    "{n} photos cannot be edited (permissions error)." : "{n}фотографии не могут быть отредактированы (ошибка доступа)",
    "Date / Time" : "Дата и время",
    "Collaborative Tags" : "Метки совместной работы",
    "EXIF Fields" : "Поля EXIF",
    "Geolocation" : "Геопозиция",
    "Orientation (EXIF)" : "Ориентация (EXIF)",
    "This feature rotates images losslessly by updating the EXIF metadata. This approach is known to sometimes not work correctly on certain image types such as HEIC. Make sure you do a test run before using it on multiple images." : "Эта функция поворачивает изображения без потерь, обновляя метаданные EXIF. Известно, что этот подход иногда некорректно работает с некоторыми типами изображений, например HEIC. Прежде чем использовать эту функцию для нескольких изображений, обязательно выполните тестовый запуск.",
    "Rotate Left" : "Повернуть налево",
    "Rotate Right" : "Повернуть направо",
    "Flip" : "Отразить",
    "Only user \"{user}\" can delete this person" : "Удалить этого человека может только пользователь \"{user}\"",
    "Remove person" : "Удалить",
    "Are you sure you want to remove {name}?" : "Вы уверены, что хотите удалить {name}?",
    "Failed to rename {oldName} to {name}." : "Не удалось переименовать «{oldName}» в «{name}».",
    "Rename person" : "Переименовать",
    "Name" : "Имя",
    "Update" : "Обновление",
    "Failed to load faces" : "Не удалось загрузить лица",
    "Create a new face with this name?" : " Создать новое лицо с таким именем?",
    "Create new face" : "Создать новое лицо",
    "Failed to create face" : "Не удалось создать лицо",
    "Loading …" : "Загрузка …",
    "Merge faces" : "Объединить лица",
    "unnamed person" : "безымянный человек",
    "Merge {name} with {newName}?" : "Объединить {name} с {newName}?",
    "You can only merge with a person with name" : "Вы можете объединить только с человеком с именем",
    "Too many failures, aborting" : "Произошло множество ошибок, действие прервано",
    "Error while moving {basename}" : "Ошибка при перемещении {basename}",
    "Failed to move {name}." : "Не удалось переместить «{name}».",
    "Merge {name} with person" : "Объединить {name} с человеком",
    "Move the selected photos to {target}?" : "Переместить выбранные фото на {target}?",
    "Move" : "Переместить",
    "An error occurred while moving photos from {name}." : "Произошла ошибка при перемещении фотографий из {name}.",
    "Move selected photos to person" : "Переместить выбранные фото на человека",
    "Choose a folder" : "Выберите папку",
    "_{n} item copied to folder_::_{n} items copied to folder_" : ["{n}элемент, скопирован в папку","{n} элемента, скопировано в папку","{n} элементов, скопировано в папку","{n} элементов, скопировано в папку"],
    "_{n} item moved to folder_::_{n} items moved to folder_" : ["{n} элемент, перемещен в папку","{n} элемента, перемещено в папку","{n} элементов, перемещено в папку","{n} элементов, перемещено в папку"],
    "Add a root to your timeline" : "Добавить корневую папку для Ленты",
    "Remove" : "Исключить",
    "Add Path" : "Добавть путь",
    "Password protected" : "Защищено паролем",
    "Expires" : "Завершение",
    "Editable" : "Редактируемый",
    "Read only" : "Только для чтения",
    "Link copied to clipboard" : "Ссылка скопирована в буфер обмена",
    "Failed to copy link to clipboard" : "Не удалось скопировать ссылку в буфер обмена",
    "Link Sharing" : "Поделиться",
    "You cannot share the root folder" : "Корневая папка не может быть опубликована",
    "Public link shares are available to people outside Nextcloud." : "Общие ссылки доступны для пользователей за пределами Nextcloud.",
    "You may create or update permissions on public links using the sidebar." : "Вы можете создавать или обновлять разрешения для публичных ссылок с помощью боковой панели.",
    "Click a link to copy to clipboard." : "Нажмите, чтобы скопировать ссылку.",
    "Share link" : "Поделиться ссылкой",
    "Create Link" : "Создать ссылку",
    "Refresh" : "Обновить",
    "Not allowed to share file: {name}" : "Не разрешен общий доступ к файлу: {name}",
    "Failed to create album for public link" : "Не удалось создать альбом по общедоступной ссылке",
    "Failed to download file {href}" : "Не удалось загрузить файл {href}",
    "Cannot share this type of data" : "Невозможно опубликовать такой тип данных",
    "Reduced Size" : "Уменьшенный размер",
    "Share in lower quality (small file size)" : "Поделиться в низком качестве (небольшой размер файла)",
    "High Resolution" : "Высокое разрешение",
    "Share in high quality (large file size)" : "Поделиться в высоком качестве (большой размер файла)",
    "Original File" : "Исходный файл",
    "Public Link" : "Общедоступная ссылка",
    "Share an external Nextcloud link" : "Поделитесь внешней ссылкой Nextcloud",
    "_Share File_::_Share Files_" : ["Поделиться","Поделиться","Поделиться","Поделиться"],
    "_Share the original file_::_Share the original files_" : ["Опубликовать оригинальный файл","Опубликовать оригинальных файла","Опубликовать оригинальные файлы","Опубликовать оригинальных файлов"],
    "No albums selected" : "Альбомы не выбраны.",
    "Choose the destination folder for the upload" : "Выберите папку назначения для загрузки",
    "Creating tags" : "Создание тегов",
    "Uploading {file}" : "Загрузка {file}",
    "Failed to upload {file}" : "Ошибка при загрузке {file}",
    "Adding files to album {album}" : "Добавление файлов в альбом {album}",
    "Attaching tags to {file}" : "Прикрепление меток к {file}",
    "Failed to attach tags to {file}" : "Не удалось прикрепить метки к {file}",
    "Some files have not been uploaded." : "Некоторые файлы не были загружены",
    "Destination path" : "Путь назначения",
    "Add to albums" : "Добавить в альбомы.",
    "Add tags" : "Добавить теги",
    "Attach collaborative tags to all uploads" : "Прикреплять теги для совместной работы ко всем загрузкам",
    "_Upload {n} file_::_Upload {n} files_" : ["Передать на сервер {n} ","Передать на сервер {n} файла","Передать на сервер {n} файлов","Передать на сервер {n} файлов"],
    "Sorting order" : "Сортировка",
    "Last updated" : "Последнее обновление:",
    "Creation date" : "Дата создания",
    "Album name" : "Название альбома",
    "Oldest first" : "Самый старый первый",
    "Ascending" : "По возрастанию",
    "Newest first" : "Самый новый первый",
    "Descending" : "По убыванию",
    "Share album" : "Опубликовать альбом",
    "Download album" : "Скачать альбом",
    "Remove album" : "Удалить альбом",
    "Nothing to show here" : "Нет ни одного объекта для просмотра",
    "Unassigned faces" : "Неназначенные лица",
    "Unnamed person" : "Безымянный человек",
    "Merge with different person" : "Объединить с другой меткой",
    "Mark person in preview" : "Отметить человека в предварительном просмотре",
    "Home" : "Дом",
    "Share folder" : "Поделиться папкой",
    "Upload files" : "Отправить файлы",
    "Folder view" : "Просмотр в виде папок",
    "Timeline view" : "Просмотр в виде Ленты",
    "Move left" : "Переместить влево",
    "Move right" : "Переместить вправо",
    "Unidentified location" : "Неустановленное местоположение",
    "Files without location" : "Файлы без местоположения",
    "Upload not permitted" : "Загрузка не разрешена",
    "_Skipped {n} file that already exists_::_Skipped {n} files that already exist_" : ["Пропущен {n}файл, уже имеющийся в системе","Пропущено {n}файла , уже имеющихся в системе","Пропущено {n}файлов, уже имеющихся в системе","Пропущено {n}файлов, уже имеющихся в системе"],
    "Uploading {file} ({current}/{total})" : "Загрузка {file} ({current}/{total}).",
    "Upload failed" : "Ошибка передачи на сервер",
    "_Successfully uploaded {n} file_::_Successfully uploaded {n} files_" : ["Успешно загружен {n} файл","Успешно загружено {n} файла","Успешно загружено {n}файлов","Успешно загружено {n}файлов"],
    "Uploaded {success} files, {failed} failed" : "Загружено {success}файлов, {failed} - ошибка",
    "_Failed to upload {n} file_::_Failed to upload {n} files_" : ["{n} не загружен.","{n} файла не загружено","{n} файлов не загружено","{n} файлов не загружено"],
    "Image saved successfully" : "Изображение сохранено",
    "Error saving image: {error}" : "Не удалось сохранить изображение: {error}",
    "Unsaved changes" : "Несохранённые изменения",
    "Drop changes" : "Отменить изменения",
    "Play Live Photo" : "Воспроизвести живое фото",
    "Download Video" : "Скачать видео",
    "Download {ext}" : "Скачать {ext}",
    "Slideshow" : "Слайд шоу",
    "Previous" : "Назад",
    "Next" : "Далее",
    "Editing is currently disabled for Live Photos" : "Редактирование живых фотографий в настоящее время отключено",
    "You are now logged in to the server!" : "Теперь вы вошли в систему на сервере!",
    "You can set up automatic uploads from this device using the Nextcloud mobile app. Click the button below to download the app, or skip this step and continue." : "Вы можете настроить автоматическую загрузку с этого устройства с помощью мобильного приложения Nextcloud. Нажмите кнопку ниже, чтобы загрузить приложение, или пропустите этот шаг и продолжите.",
    "Set up automatic upload" : "Настроить автоматическую загрузку",
    "Memories can show local media on your device alongside the media on your server. This requires access to the media on this device." : "В «Воспоминаниях» можно просматривать локальные медиафайлы на вашем устройстве вместе с медиафайлами на вашем сервере. Для этого требуется доступ к медиафайлам на этом устройстве.",
    "Access to media has been granted." : "Доступ к фото\\видео предоставлен.",
    "Access to media is not available yet. If the button below does not work, grant the permission through settings." : "Доступ к медиа пока недоступен. Если кнопка ниже не работает, предоставьте разрешение через настройки.",
    "Grant permissions" : "Предоставить разрешения",
    "Skip this step" : "Пропустить этот шаг",
    "Choose the folders on this device to show on your timeline." : "Выберите папки на этом устройстве, которые будут отображаться в вашей Ленте",
    "If no folders are visible here, you may need to grant the app storage permissions, or wait for the app to index your files." : "Если здесь не отображаются папки, возможно, вам нужно предоставить приложению доступ к хранилищу или дождаться, пока приложение проиндексирует ваши файлы.",
    "You can always change this in settings. Note that this does not affect automatic uploading." : "Вы всегда можете изменить это в настройках. Обратите внимание, что это не влияет на автоматическую загрузку.",
    "Synchronizing local files ({n} done)." : "Синхронизация локальных файлов ({n} выполнено)",
    "This may take a while. Do not close this window." : "Действие может занять время. Не закрывайте это окно.",
    "Finish" : "Завершить",
    "{name} binary exists and is executable." : "Двоичный файл {name} существует и является исполняемым.",
    "{name} binary not found." : "Двоичный файл {name} не найден.",
    "{name} binary is not executable." : "Двоичный файл {name} не является исполняемым.",
    "{name} failed test: {info}." : "{name} сбой теста: {info}",
    "{name} binary exists and is usable ({info})." : "Двоичный файл {name} существует и пригодный для использования ({info})",
    "{name} binary status: {status}." : "Состояние двоичного файла: {status}.",
    "Save as" : "Сохранить как",
    "Are you sure you want to discard the changes?" : "Вы действительно хотите отменить изменения?",
    "Undo" : "Отменить",
    "Redo" : "Повторить",
    "Show original image" : "Показать исходное изображение",
    "Zoom in" : "Приблизить",
    "Zoom out" : "Отдалить",
    "Toggle zoom menu" : "Меню мастшабирования",
    "Adjust" : "Настроить",
    "Fine-tune" : "Обработка",
    "Filters" : "Фильтры",
    "Watermark" : "Водяной знак",
    "Draw" : "Рисование",
    "Resize" : "Изменение размера",
    "Invalid image." : "Неверный формат изображения.",
    "Error while uploading the image." : "Не удалось передать изображение на сервер.",
    "are not images" : "не являются изображениями",
    "is not an image" : "не является изображением",
    "to be uploaded" : "для загрузки на сервер",
    "Crop" : "Обрезать",
    "Original" : "Исходное",
    "Custom" : "Свои параметры",
    "Square" : "Квадрат",
    "Landscape" : "Ландшафт",
    "Portrait" : "Портрет",
    "Ellipse" : "Эллипс",
    "Classic TV" : "Классическое ТВ",
    "CinemaScope" : "Синемаскоп",
    "Arrow" : "Стрела",
    "Blur" : "Размытие",
    "Brightness" : "Яркость",
    "Contrast" : "Контраст",
    "Un-flip X" : "Отразить по горизонтали",
    "Flip X" : "Отразить по горизонтали",
    "Un-flip Y" : "Отразить по вертикали",
    "Flip Y" : "Отразить по вертикали",
    "HSV" : "HSV (Тон, насыщенность, значение)",
    "Hue" : "Тон",
    "Saturation" : "Насыщенность",
    "Value" : "Значение",
    "Image" : "Изображение",
    "Importing …" : "Импорт…",
    "+ Add image" : "+ Добавить изображение",
    "Line" : "Линия",
    "Pen" : "Ручка",
    "Polygon" : "Многоугольник",
    "Sides" : "Количество сторон",
    "Rectangle" : "Прямоугольник",
    "Corner Radius" : "Радиус скругления",
    "Width in pixels" : "Тощина (пикс.)",
    "Height in pixels" : "Высота (пикс.)",
    "Toggle ratio lock" : "Блокировка соотношения сторон",
    "Reset to original image size" : "Вернуться с исходному размеру",
    "Rotate" : "Поворот",
    "Text" : "Текст",
    "Text spacing" : "Межстрочный интервал",
    "Text alignment" : "Выравнивание текста",
    "Font family" : "Гарнитура шрифта",
    "Size" : "Размер",
    "Letter spacing" : "Межсимвольный интервал",
    "Line height" : "Высота строки",
    "Warmth" : "Теплота",
    "+ Add watermark" : "+ Добавить водяной знак",
    "Choose watermark type" : "Выберите тип водяного знака",
    "Upload watermark" : "Загрузить водяной знак",
    "Add as text" : "Добавить как текст",
    "Padding" : "Отступы",
    "Shadow" : "Тень",
    "Horizontal" : "По горизонтали",
    "Vertical" : "По вертикали",
    "Opacity" : "Непрозрачность",
    "Position" : "Должность",
    "Stroke" : "Зачёркнутый",
    "Save image as" : "Сохранить изображение как",
    "Extension" : "Расширение",
    "Name is required." : "Необходимо указать имя.",
    "Quality" : "Качество",
    "Saved image size (width x height)" : "Размер сохранённого изображения (ширина × высота)",
    "Note that the selected crop area is lower than the applied resize which might cause quality decrease" : "Обратите внимание, что выбранная область обрезки меньше примененного изменения размера, что может привести к снижению качества",
    "Actual size (100%)" : "Фактический размер (100%)",
    "Fit size" : "По размеру окна",
    "Transcoding failed, check Nextcloud logs." : "Не удалось перекодировать, проверьте журналы Nextcloud.",
    "Direct" : "Без перекодирования",
    "Auto" : "Автоматически",
    "Shared Folder" : "Опубликованная Папка",
    "Shared Album" : "Опубликованный альбом",
    "Setup" : "Настройка",
    "Failed to add {filename} to album." : "Не удалось добавить {filename}в альбом",
    "Failed to remove {filename}." : "Не удалось удалить {filename}",
    "Failed to create {albumName}." : "Не удалось создать альбом «{albumName}».",
    "Failed to update properties of {albumName} with {properties}." : "Не удалось обновить свойства {albumName} на {properties}.",
    "Failed to rename {currentAlbumName} to {newAlbumName}." : "Не удалось переименовать альбом «{currentAlbumName}» в «{newAlbumName}».",
    "No items" : "Нет ни одного объекта",
    "_{n} item_::_{n} items_" : ["{n} объект","{n} объекта","{n} объектов","{n} объектов"],
    "Shared by {user}" : "Опубликовано пользователем {user} ",
    "General Failure" : "Общий сбой",
    "Error: {msg}" : "Ошибка: {msg}",
    "Failed to delete {fileName}." : "Ошибка удаления файла «{fileName}».",
    "Failed to copy files." : "Не удалось скопировать файлы.",
    "Could not copy {fileName}, target exists." : "Ошибка копирования {fileName}, файл уже существует.",
    "Failed to copy {fileName}." : "Не удалось скопировать {fileName}.",
    "Failed to move files." : "Не удалось переместить файлы.",
    "Could not move {fileName}, target exists." : "Ошибка перемещения {fileName}, файл уже существует",
    "Failed to move {fileName}." : "Не удалось переместить {fileName}",
    "Failed to load image info: {name}" : "Не удалось загрузить информацию об изображении: {name}",
    "Failed to download files" : "Не удалось скачать файлы",
    "Failed to remove {filename} from face." : "Не удалось удалить {filename} из распознанного лица.",
    "Failed to move {filename} from face." : "Не удалось переместить {filename} из лица.",
    "Failed to favorite files." : "Не удалось добавить файлы в избранное.",
    "Failed to favorite some files." : "Некоторые файлы не удалось добавить в избранное ",
    "Failed to favorite {fileName}." : "Не удалось добавить файл «{fileName}» в избранное.",
    "Cover image set successfully" : "Обложка успешно установлена",
    "Failed to set cover image" : "Ошибка при установке обложки",
    "No content-location header found" : "Заголовок content-location не найден",
    "Failed to create tag {name}: {error}" : "Не удалось создать метку «{name}»: {error}",
    "Memories has been updated to {version}. Reload to get the new version." : "Memories была обновлена до {version}. Перезагрузите, чтобы получить новую версию.",
    "Upload some photos and make sure the timeline path is configured" : "Загрузите несколько фотографий и убедитесь, что путь к Ленте настроен",
    "Mark photos as favorite to find them easily" : "Отмечайте фотографии как избранные, чтобы легко их находить",
    "Memories from past years will appear here" : "Здесь появятся воспоминания из прошлых лет",
    "Recognize is still working on your photos" : "Программа распознавания все еще работает с вашими фотографиями",
    "You will find your friends soon. Please be patient" : "Скоро вы найдете своих друзей. Пожалуйста, наберитесь терпения.",
    "Face Recognition is disabled. Enable in settings to find your friends" : "Распознавание лиц отключено. Включите его в настройках для поиска своих друзей.",
    "Your videos will appear here" : "Ваши видео появятся здесь",
    "Add photos to albums by selecting them on your timeline." : "Чтобы добавить фото в альбомы, выберите их в Ленте",
    "Create an album to get started" : "Создайте альбом, чтобы начать работу",
    "Archive photos you don't want to see in your timeline" : "Архивируйте фотографии, которые вы не хотите видеть в Ленте",
    "Tag photos to find them easily" : "Помечайте фотографии тегами, чтобы легко их находить",
    "Places you have been to will appear here" : "Места, где вы побывали, появятся здесь",
    "Yes" : "Да",
    "No" : "Нет",
    "Choose {file}" : "Выбран «{file}»",
    "Choose" : "Выберите",
    "The folder name \"{folder}\" has a leading or trailing whitespace. This may lead to errors and should be corrected." : "В имени папки \"{folder}\" есть пробел в начале или в конце. Это может привести к ошибкам, необходимо исправить.",
    "_Move {count} item to trash?_::_Move {count} items to trash?_" : ["Переместить {count} в корзину?","Переместить {count} в корзину?","Переместить {count} в корзину?","Переместить {count} в корзину?"],
    "Files in trash may be automatically deleted after a fixed period of time." : "Файлы в корзине, могут быть автоматически удалены по истечении определенного периода времени.",
    "_Remove {count} item from album?_::_Remove {count} items from album?_" : ["Удалить {count} из альбома?","Удалить {count} из альбома?","Удалить {count} из альбома?","Удалить {count} из альбома?"],
    "This will not delete your original files." : "Это не приведет к удалению исходных файлов.",
    "You are about to download {count} items." : "Подтвердите загрузку {count} объекта (-ов).",
    "You are about to move {count} items." : "Подтвердите перемещение {count} объекта (-ов)."
},
"nplurals=4; plural=(n%10==1 && n%100!=11 ? 0 : n%10>=2 && n%10<=4 && (n%100<12 || n%100>14) ? 1 : n%10==0 || (n%10>=5 && n%10<=9) || (n%100>=11 && n%100<=14)? 2 : 3);");
